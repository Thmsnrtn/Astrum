...
export function MoonDisc({ elong, R = 48 }) {
  ...
  return (
    <svg width={R * 2 + 6} height={R * 2 + 6} viewBox={`${-R - 3} ${-R - 3} ${R * 2 + 6} ${R * 2 + 6}`}>
      <circle r={R} fill="#0a0918" stroke="rgba(var(--tint-rgb),0.22)" strokeWidth="1" />
      <path d={d} fill="#E8DFC0" />
      <circle r={R} fill="none" stroke="rgba(var(--tint-rgb),0.35)" strokeWidth="0.6" />
    </svg>
  );
}
...
function Hinge({ label, date, keynote, col, big }) {
  ...
  return (
    <div style={{ display: "flex", gap: 11, alignItems: "flex-start", padding: big ? "11px 13px" : "8px 12px", borderRadius: 11, background: big ? col + "12" : "rgba(8,5,22,0.5)", border: `1px solid ${big ? col + "3A" : "rgba(var(--tint-rgb),0.1)"}`, marginBottom: 7 }}>
      <div style={{ flex: 1 }}>
      ...
      <div style={{ textAlign: "right", flexShrink: 0 }}>
        ...
        <div style={{ fontFamily: F, fontSize: 8.5, color: "rgba(var(--tint-rgb),0.45)", marginTop: 1 }}>{when}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", paddingBottom: 20 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 14px" }}>
        {/* Current phase */}
        <div style={{ display: "flex", gap: 16, alignItems: "center", padding: "14px 15px", borderRadius: 14, background: "rgba(8,5,22,0.8)", border: "1px solid rgba(var(--tint-rgb),0.15)", marginBottom: 9 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontFamily: F, fontSize: 9.5, color: "rgba(var(--tint-rgb),0.5)", marginTop: 3 }}>Day {Math.round(tl.ageDays)} of {Math.round(29.53)}</div>
        ...
        <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.4)", letterSpacing: 2.5, textTransform: "uppercase", margin: "4px 4px 7px" }}>The Coming Turns</div>
        ...
        <Hinge big label="○ New Moon — plant the seed" date={tl.nextNew} keynote="Set the next lunation's intention; begin." col={GOLD} />
        ...
        <div className="card" style={{ marginTop: 4, marginBottom: 9 }}>
          ...
          <button onClick={plantIntention} disabled={!intent.trim() || saved} style={{ width: "100%", marginTop: 8, padding: "11px 0", borderRadius: 11, background: saved ? "rgba(92,168,92,0.15)" : intent.trim() ? "rgba(var(--tint-rgb),0.12)" : "rgba(0,0,0,0.3)", border: `1px solid ${saved ? "rgba(92,168,92,0.4)" : intent.trim() ? "rgba(var(--tint-rgb),0.35)" : "rgba(var(--tint-rgb),0.1)"}`, fontFamily: F, fontSize: 9.5, color: saved ? "#7AB07A" : intent.trim() ? GOLD : "#5A4020", letterSpacing: 2, textTransform: "uppercase", cursor: intent.trim() && !saved ? "pointer" : "default" }}>{saved ? "✓ Planted — judge it at the Full, in Review" : "⚑ Plant This Intention"}</button>
        </div>
        ...
        {openCastings.length > 0 && (
          <div style={{ padding: "12px 14px", borderRadius: 13, background: "rgba(8,5,22,0.6)", border: "1px solid rgba(var(--tint-rgb),0.1)", marginBottom: 9 }}>
            <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.45)", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>To Bring to Light at the Full · {openCastings.length} open</div>
            {openCastings.map(c => (
              <div key={c.id} style={{ display: "flex", gap: 8, padding: "5px 0", borderBottom: "1px solid rgba(var(--tint-rgb),0.05)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 5, marginBottom: 9 }}>
          {PHASES.map((p, i) => (
            <div key={p.key} style={{ padding: "7px 6px", borderRadius: 9, background: i === ph.index ? GOLD + "18" : "rgba(0,0,0,0.25)", border: `1px solid ${i === ph.index ? GOLD + "50" : "rgba(var(--tint-rgb),0.08)"}`, textAlign: "center" }}>
