...
export default function AltarScreen({ now, hour, eph, setTab }) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "20px 18px", position: "relative", background: `radial-gradient(ellipse at 50% 38%, ${p.col || GOLD}0A, transparent 65%)` }}>
      {/* Clock */}
      <div style={{ fontFamily: F, fontSize: 13, color: "rgba(var(--tint-rgb),0.45)", letterSpacing: 3 }}>{clock.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })}</div>
      ...
      {/* The hour */}
      <div style={{ fontSize: 110, color: p.col || GOLD, lineHeight: 1.15, marginTop: 12, textShadow: `0 0 42px ${p.glow || "rgba(var(--tint-rgb),0.3)"}` }}>{p.sym}</div>
      <div style={{ fontFamily: F, fontSize: 22, color: p.col || GOLD, letterSpacing: 4, textTransform: "uppercase" }}>Hour of {p.name}</div>
      <div style={{ fontFamily: F, fontSize: 11.5, color: "rgba(var(--tint-rgb),0.5)", marginTop: 5 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 20 }}>
        ...
        <div>
          ...
          {eph?.voc?.isVoC
            ...
            : <div style={{ fontFamily: F, fontSize: 10.5, color: "rgba(var(--tint-rgb),0.4)", marginTop: 2 }}>The channel is open</div>}
        </div>
      </div>
      ...
      {(observances.length > 0 || watches.length > 0) && (
        <div style={{ marginTop: 18, textAlign: "center" }}>
          {observances.slice(0, 2).map((o, i) => (
            <div key={i} style={{ fontFamily: F, fontSize: 11, color: GOLD, padding: "2px 0" }}>🕯 {o.label} today — tend the relationship</div>
          ))}
      ...
      <div style={{ position: "absolute", bottom: 26, display: "flex", gap: 10 }}>
        <button onClick={() => setTab && setTab("rite")} style={{ padding: "11px 24px", borderRadius: 12, background: (p.col || GOLD) + "16", border: `1px solid ${(p.col || GOLD)}40`, fontFamily: F, fontSize: 10, color: p.col || GOLD, letterSpacing: 3, textTransform: "uppercase", cursor: "pointer" }}>✦ Begin a Rite</button>
        {soundAvailable() && (
          <button onClick={() => { setBellOn(b => !b); if (!bellOn) bell(432); }} style={{ padding: "11px 18px", borderRadius: 12, background: bellOn ? "rgba(var(--tint-rgb),0.14)" : "rgba(0,0,0,0.3)", border: `1px solid ${bellOn ? "rgba(var(--tint-rgb),0.4)" : "rgba(var(--tint-rgb),0.15)"}`, fontFamily: F, fontSize: 10, color: bellOn ? GOLD : "rgba(var(--tint-rgb),0.5)", letterSpacing: 2, textTransform: "uppercase", cursor: "pointer" }}>{bellOn ? "🔔 Hour bell on" : "🔕 Hour bell"}</button>
        )}
      </div>
    </div>
  );
