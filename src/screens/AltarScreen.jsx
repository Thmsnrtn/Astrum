...
export default function AltarScreen({ now, hour, eph, setTab }) {
  ...
  const observances = upcomingObservances(loadSpirits(), dayStart, 1);
  const watches = loadWatchlist().filter(w => w.active && w.nextWindow && new Date(w.nextWindow.date) > now && new Date(w.nextWindow.date) - now < 48 * 3600000);
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "20px 18px", position: "relative", background: `radial-gradient(ellipse at 50% 38%, ${p.col || "#D4AF6A"}0A, transparent 65%)` }}>
      {/* Clock */}
      <div style={{ fontFamily: F, fontSize: 13, color: "rgba(200,175,100,0.45)", letterSpacing: 3 }}>{now.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })}</div>
      <div style={{ fontFamily: F, fontSize: 30, color: "rgba(212,192,152,0.85)", letterSpacing: 2, marginTop: 2 }}>{now.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</div>
      ...
      <div style={{ fontFamily: F, fontSize: 11.5, color: "rgba(200,175,100,0.5)", marginTop: 5 }}>
        {hour?.msRemaining != null ? `${fmtHM(hour.msRemaining)} remaining` : ""} · then {P[hour?.nextPlanet]?.name || ""} · Day of {P[hour?.dayRuler]?.name || ""}
      </div>
