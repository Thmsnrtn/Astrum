...
function CondChips({ c }) {
  ...
  if (!cond) return <span style={{ fontFamily: F, fontSize: 8, color: "rgba(200,175,100,0.25)", fontStyle: "italic" }}>no sky record</span>;
  const chips = [];
  ...
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 5 }}>
      {chips.map((t, i) => (
        <span key={i} style={{ fontFamily: F, fontSize: 7.5, color: "rgba(200,175,100,0.5)", letterSpacing: 0.8, padding: "2px 7px", borderRadius: 8, background: "rgba(0,0,0,0.3)", border: "1px solid rgba(200,175,100,0.1)" }}>{t}</span>
      ))}
    </div>
  );
}
      ...
      <div style={{ marginTop: 8 }}>
        {shown.map(r => (
          <div key={r.key} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
            ...
            <div style={{ fontFamily: F, fontSize: 9, color: "rgba(200,175,100,0.55)", width: 62, textAlign: "right", flexShrink: 0 }}>{r.pct != null ? `${r.pct}%` : "—"} · n={r.n}</div>
          </div>
        ))}
      </div>
    </div>
...
export default function ReviewScreen({ profile }) {
  ...
  const CastingRow = ({ c, showVerdict }) => {
    ...
    return (
      <div style={{ marginBottom: 8, padding: "11px 12px", borderRadius: 13, background: "rgba(8,5,22,0.65)", border: `1px solid ${pl ? pl.col + "20" : "rgba(200,175,100,0.1)"}` }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", cursor: "pointer" }} onClick={() => { setExpanded(isExp ? null : c.id); setNote(""); }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              ...
              <span style={{ fontFamily: F, fontSize: 8, color: "rgba(200,175,100,0.45)", letterSpacing: 1.5, textTransform: "uppercase" }}>{c.kind}</span>
              <span style={{ fontFamily: F, fontSize: 9, color: "rgba(200,175,100,0.35)" }}>{new Date(c.createdAt).toLocaleDateString()}</span>
          ...
          <span style={{ color: "rgba(200,175,100,0.3)", fontSize: 11, marginLeft: 6 }}>{isExp ? "▾" : "▸"}</span>
        </div>
        {isExp && (
          <div style={{ marginTop: 9, paddingTop: 9, borderTop: "1px solid rgba(200,175,100,0.08)" }}>
            {(c.outcomes || []).map(o => (
              <div key={o.id} style={{ marginBottom: 6, padding: "6px 9px", borderRadius: 8, background: "rgba(0,0,0,0.3)" }}>
                <span style={{ fontFamily: F, fontSize: 8, color: VERDICT_META[o.verdict]?.col || "rgba(200,175,100,0.4)", letterSpacing: 1 }}>{(o.verdict || "note").toUpperCase()}</span>
                <span style={{ fontFamily: F, fontSize: 9, color: "rgba(200,175,100,0.35)", marginLeft: 6 }}>{new Date(o.date).toLocaleDateString()}</span>
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
              ...
              <button onClick={() => record(c.id, "unknown")} style={{ flex: 1, padding: "8px 0", borderRadius: 9, background: "rgba(0,0,0,0.3)", border: "1px solid rgba(200,175,100,0.15)", fontFamily: F, fontSize: 9, color: "rgba(200,175,100,0.5)", letterSpacing: 1, cursor: "pointer" }}>NOTE ONLY</button>
            </div>
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", paddingBottom: 20 }}>
      ...
      <div style={{ display: "flex", gap: 6, padding: "6px 14px 10px" }}>
        {TABS.map(([id, lbl]) => (
          <button key={id} onClick={() => setView(id)} style={{ flex: 1, padding: "8px 0", borderRadius: 10, background: view === id ? "rgba(212,175,106,0.12)" : "rgba(0,0,0,0.25)", border: `1px solid ${view === id ? "rgba(212,175,106,0.35)" : "rgba(200,175,100,0.08)"}`, fontFamily: F, fontSize: 9, color: view === id ? GOLD : "rgba(200,175,100,0.4)", letterSpacing: 1, cursor: "pointer" }}>{lbl}</button>
        ))}
      </div>
          ...
          <>
            <div className="card" style={{ margin: "0 14px 10px", display: "flex", justifyContent: "space-around", textAlign: "center" }}>
              {[["Castings", stats.total], ["Judged", stats.judged], ["Open", stats.open], ["Hit Rate", stats.overall.pct != null ? `${stats.overall.pct}%` : "—"]].map(([lbl, val]) => (
                <div key={lbl}>
                  ...
                  <div style={{ fontFamily: F, fontSize: 7.5, color: "rgba(200,175,100,0.4)", letterSpacing: 1.5, textTransform: "uppercase", marginTop: 2 }}>{lbl}</div>
                </div>
              ))}
            </div>
            {stats.judged < 3 ? (
            ...
            {analysis && (
              <div style={{ margin: "0 14px 10px", padding: "13px 14px", borderRadius: 13, background: "rgba(20,15,40,0.8)", border: "1px solid rgba(100,80,160,0.25)" }}>
                ...
                <button onClick={() => setAnalysis(null)} style={{ marginTop: 10, background: "none", border: "none", fontFamily: F, fontSize: 9, color: "rgba(200,175,100,0.3)", cursor: "pointer", letterSpacing: 1 }}>DISMISS</button>
              </div>
            )}
          </>
        )}
