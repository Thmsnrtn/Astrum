...
function PeriodRow({ p, current, onClick, sub }) {
  ...
  return (
    <div onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 9, padding: sub ? "6px 10px" : "9px 12px", borderRadius: 10, background: current ? "rgba(var(--tint-rgb),0.12)" : "rgba(8,5,22,0.55)", border: `1px solid ${current ? "rgba(var(--tint-rgb),0.45)" : "rgba(var(--tint-rgb),0.08)"}`, marginBottom: 5, cursor: onClick ? "pointer" : "default" }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontFamily: F, fontSize: sub ? 8.5 : 9, color: "rgba(var(--tint-rgb),0.45)", marginTop: 1 }}>{sub ? `${fmtDay(p.start)} — ${fmtDay(p.end)}` : `${fmtD(p.start)} — ${fmtD(p.end)}`}{p.truncated ? " ·" : ""}</div>
      </div>
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", paddingBottom: 20 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 14px" }}>
        ...
        {ready && prof && (
          <div style={{ padding: "13px 15px", borderRadius: 13, background: "rgba(8,5,22,0.75)", border: "1px solid rgba(var(--tint-rgb),0.15)", marginBottom: 10 }}>
            <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.5)", letterSpacing: 2, textTransform: "uppercase", marginBottom: 6 }}>The Year · Annual Profection · Age {prof.age}</div>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {natalPos?.[prof.lord] && <div style={{ fontFamily: F, fontSize: 9, color: "rgba(var(--tint-rgb),0.5)", marginTop: 3 }}>Natal {P[prof.lord].name}: {natalPos[prof.lord].zodiac?.degree}° {natalPos[prof.lord].zodiac?.name} ({natalPos[prof.lord].dignity})</div>}
              </div>
            </div>
          </div>
        )}
          ...
          <>
            <div style={{ display: "flex", gap: 6, marginBottom: 9 }}>
              {[["spirit", "☉ From Spirit — career & action"], ["fortune", "☽ From Fortune — body & fortune"]].map(([k, lbl]) => (
                <button key={k} onClick={() => { setLotKey(k); setOpenL1(null); }} style={{ flex: 1, padding: "9px 0", borderRadius: 10, background: lotKey === k ? "rgba(var(--tint-rgb),0.14)" : "rgba(0,0,0,0.3)", border: `1px solid ${lotKey === k ? "rgba(var(--tint-rgb),0.4)" : "rgba(var(--tint-rgb),0.1)"}`, fontFamily: F, fontSize: 8.5, color: lotKey === k ? GOLD : "#7A6030", letterSpacing: 1, textTransform: "uppercase", cursor: "pointer" }}>{lbl}</button>
              ))}
            </div>

            {/* The current chain */}
            {zr.l1 && (
              <div style={{ padding: "12px 14px", borderRadius: 13, background: "rgba(8,5,22,0.75)", border: `1px solid ${zr.l1.angle ? "rgba(122,176,122,0.35)" : "rgba(var(--tint-rgb),0.15)"}`, marginBottom: 10 }}>
                <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.5)", letterSpacing: 2, textTransform: "uppercase", marginBottom: 6 }}>The Chapter Now · releasing from the Lot of {lotKey === "spirit" ? "Spirit" : "Fortune"} in {SIGN_NAMES[lotSign]}</div>
                ...
                <div style={{ fontFamily: F, fontSize: 9.5, color: "rgba(var(--tint-rgb),0.5)", marginTop: 2 }}>{fmtD(zr.l1.start)} — {fmtD(zr.l1.end)} · ruled by {P[["mars","venus","mercury","moon","sun","mercury","venus","mars","jupiter","saturn","saturn","jupiter"][zr.l1.sign]]?.name}</div>
            ...
            {/* L1 timeline */}
            <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.4)", letterSpacing: 2.5, textTransform: "uppercase", margin: "2px 4px 7px" }}>The Major Periods</div>
