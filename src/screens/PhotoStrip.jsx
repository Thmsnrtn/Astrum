      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        {photoIds.map(id => thumbs[id] ? (
          <div key={id} style={{ position: "relative" }}>
            <img src={thumbs[id]} onClick={() => setView(id)} alt="" style={{ width: 54, height: 54, objectFit: "cover", borderRadius: 9, border: "1px solid rgba(200,175,100,0.25)", cursor: "pointer", display: "block" }} />
          </div>
        ) : <div key={id} style={{ width: 54, height: 54, borderRadius: 9, background: "rgba(0,0,0,0.3)", border: "1px solid rgba(200,175,100,0.1)" }} />)}
        <label style={{ width: 54, height: 54, borderRadius: 9, background: "rgba(212,175,106,0.08)", border: "1px dashed rgba(212,175,106,0.35)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontFamily: F, fontSize: 18, color: "rgba(212,175,106,0.7)" }}>
          {busy ? "…" : "+"}
          <input type="file" accept="image/*" capture="environment" onChange={add} style={{ display: "none" }} />
        </label>
        <span style={{ fontFamily: F, fontSize: 8, color: "rgba(200,175,100,0.35)", letterSpacing: 1.5, textTransform: "uppercase" }}>{label}</span>
      </div>
      {view && thumbs[view] && (
        <div onClick={() => setView(null)} style={{ position: "fixed", inset: 0, zIndex: 500, background: "rgba(0,0,0,0.92)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          <img src={thumbs[view]} alt="" style={{ maxWidth: "94%", maxHeight: "84%", borderRadius: 12, border: "1px solid rgba(200,175,100,0.3)" }} />
