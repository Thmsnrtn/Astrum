...
export default function AthanorScreen({ profile, natalPos, eph, now }) {
  ...
  const Header = () => (
    <div style={{ padding: "16px 18px 6px", display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
      ...
      {chamber === "fire" && mode === "list" && (
        <button onClick={() => setMode("new")} style={{ padding: "8px 14px", borderRadius: 10, background: "rgba(var(--tint-rgb),0.1)", border: "1px solid rgba(var(--tint-rgb),0.28)", fontFamily: F, fontSize: 9, color: GOLD, letterSpacing: 2, cursor: "pointer" }}>+ OPERATION</button>
      )}
    </div>
  );
  ...
  if (chamber === "fire" && mode === "new") {
    ...
    return (
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 14px 30px" }}>
        <button onClick={() => setMode("list")} style={{ background: "none", border: "none", color: "rgba(var(--tint-rgb),0.5)", fontFamily: F, fontSize: 10, letterSpacing: 2, cursor: "pointer", marginBottom: 14, padding: 0 }}>← THE FIRE</button>
        <div style={T(18)}>Light the Fire</div>
        <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.4)", letterSpacing: 2, textTransform: "uppercase", margin: "14px 0 6px" }}>Operation</div>
        {TEMPLATE_ORDER.map(tid => {
          ...
          return (
            <button key={tid} onClick={() => setTemplateId(tid)} style={{ width: "100%", textAlign: "left", marginBottom: 6, padding: "10px 12px", borderRadius: 11, background: active ? "rgba(var(--tint-rgb),0.09)" : "rgba(0,0,0,0.25)", border: `1px solid ${active ? "rgba(var(--tint-rgb),0.4)" : "rgba(var(--tint-rgb),0.08)"}`, cursor: "pointer" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                <span style={{ fontSize: 13 }}>{fam.glyph}</span>
                <span style={{ fontFamily: F, fontSize: 11.5, color: active ? GOLD : "rgba(var(--tint-rgb),0.55)", flex: 1 }}>{t.name}</span>
              ...
              <div style={{ fontFamily: F, fontSize: 9, color: "rgba(var(--tint-rgb),0.35)", fontStyle: "italic", marginTop: 3, lineHeight: 1.5 }}>{t.desc}</div>
              {active && <>
                <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.3)", marginTop: 5 }}>{fam.label} · {t.steps.length} steps · {t.source}</div>
                <div style={{ fontFamily: F, fontSize: 8.5, color: "rgba(var(--tint-rgb),0.4)", marginTop: 3 }}>Apparatus: {t.apparatus}</div>
        ...
        <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.4)", letterSpacing: 2, textTransform: "uppercase", margin: "12px 0 6px" }}>Ruling Planet</div>
        <div style={{ display: "flex", gap: 4 }}>
          {Object.keys(P).map(pk => (
            <button key={pk} onClick={() => setPlanet(pk)} style={{ flex: 1, padding: "8px 2px", borderRadius: 9, background: planet === pk ? P[pk].col + "18" : "rgba(0,0,0,0.25)", border: `1px solid ${planet === pk ? P[pk].col + "55" : "rgba(var(--tint-rgb),0.08)"}`, cursor: "pointer" }}>
              <div style={{ fontSize: 14, textAlign: "center", color: planet === pk ? P[pk].col : "rgba(var(--tint-rgb),0.3)" }}>{P[pk].sym}</div>
            </button>
          ))}
        </div>
        <div style={{ fontFamily: F, fontSize: 8.5, color: "rgba(var(--tint-rgb),0.35)", fontStyle: "italic", marginTop: 6, lineHeight: 1.6 }}>Herbs of {P[planet].name}: {P[planet].herbs} · Metal: {METALS.find(m => m.planet === planet)?.metal}</div>
        <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.4)", letterSpacing: 2, textTransform: "uppercase", margin: "12px 0 6px" }}>Name the Work (optional)</div>
        ...
        <button onClick={start} style={{ width: "100%", marginTop: 14, padding: "13px 0", borderRadius: 12, background: "rgba(var(--tint-rgb),0.12)", border: "1px solid rgba(var(--tint-rgb),0.35)", fontFamily: F, fontSize: 10, color: GOLD, letterSpacing: 3, textTransform: "uppercase", cursor: "pointer" }}>🜂 Begin the Operation</button>
        <div style={{ fontFamily: F, fontSize: 8.5, color: "rgba(var(--tint-rgb),0.35)", fontStyle: "italic", marginTop: 8, textAlign: "center", lineHeight: 1.6 }}>The first step schedules to its next qualifying window ({describeDueRule(tpl.steps[0].dueRule, P[planet].name) || "now"}); each completed step schedules the next. The sky record is written the moment you begin.</div>
      </div>
    );
  }
  ...
  if (chamber === "fire" && mode === "view" && sel) {
    ...
    return (
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 14px 30px" }}>
        <button onClick={() => { setMode("list"); setSel(null); refresh(); }} style={{ background: "none", border: "none", color: "rgba(var(--tint-rgb),0.5)", fontFamily: F, fontSize: 10, letterSpacing: 2, cursor: "pointer", marginBottom: 14, padding: 0 }}>← THE FIRE</button>
        <div style={T(18)}><span style={{ color: pl.col }}>{pl.sym}</span> {op.name}</div>
        <div style={{ fontFamily: F, fontSize: 9, color: "rgba(var(--tint-rgb),0.4)", marginTop: 3 }}>{tpl?.name} · begun {new Date(op.startedAt).toLocaleDateString()} · {op.status}</div>
        ...
        {op.status === "active" && nextStep && (
          <div style={{ marginTop: 12, padding: "13px 14px", borderRadius: 13, background: `${pl.col}0D`, border: `1px solid ${pl.col}40` }}>
            ...
            <div style={{ fontFamily: F, fontSize: 8.5, color: "rgba(var(--tint-rgb),0.45)", marginTop: 3 }}>
        ...
        {op.status === "complete" && <div style={{ marginTop: 12, padding: "11px 13px", borderRadius: 12, background: "rgba(var(--tint-rgb),0.08)", border: "1px solid rgba(var(--tint-rgb),0.3)", fontFamily: F, fontSize: 10.5, color: GOLD, fontStyle: "italic", lineHeight: 1.7 }}>The operation is complete. A summary is written in the Grimoire; Review will ask for the verdict when the work has spoken.</div>}

        <div style={{ marginTop: 12 }}>
          <div style={{ fontFamily: F, fontSize: 9, color: "rgba(var(--tint-rgb),0.5)", letterSpacing: 3, textTransform: "uppercase", marginBottom: 7 }}>Process Timeline</div>
          {op.steps.map((s, i) => (
            <div key={s.id} style={{ display: "flex", gap: 10, marginBottom: 2 }}>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: 16 }}>
                <div style={{ width: 10, height: 10, borderRadius: 5, background: s.completedAt ? pl.col : "rgba(0,0,0,0.4)", border: `1.5px solid ${s.completedAt ? pl.col : s === nextStep ? pl.col + "80" : "rgba(var(--tint-rgb),0.2)"}`, flexShrink: 0, marginTop: 3 }} />
                {i < op.steps.length - 1 && <div style={{ width: 1.5, flex: 1, minHeight: 14, background: s.completedAt ? pl.col + "50" : "rgba(var(--tint-rgb),0.1)" }} />}
              </div>
              <div style={{ paddingBottom: 12, flex: 1 }}>
                <div style={{ fontFamily: F, fontSize: 11, color: s.completedAt ? "#C4A870" : s === nextStep ? GOLD : "rgba(var(--tint-rgb),0.35)" }}>{s.title}{s.fire ? <span style={{ fontSize: 9, color: "rgba(var(--tint-rgb),0.35)" }}>  {FIRE_DEGREES[s.fire - 1]?.glyph}</span> : null}</div>
                <div style={{ fontFamily: F, fontSize: 8.5, color: "rgba(var(--tint-rgb),0.35)", marginTop: 1 }}>
        ...
        <div style={{ marginTop: 8 }}>
          <div style={{ fontFamily: F, fontSize: 9, color: "rgba(var(--tint-rgb),0.5)", letterSpacing: 3, textTransform: "uppercase", marginBottom: 4 }}>Lab Notebook</div>
          <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.3)", fontStyle: "italic", marginBottom: 7, lineHeight: 1.5 }}>“The exact keeping of a laboratory diary should be the duty of every practicing spagyrist.” — Junius</div>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            ...
            <button onClick={() => { if (labText.trim()) { addLabNote(op.id, labText.trim()); setLabText(""); refresh(); } }} style={{ padding: "0 14px", borderRadius: 10, background: "rgba(var(--tint-rgb),0.1)", border: "1px solid rgba(var(--tint-rgb),0.3)", fontFamily: F, fontSize: 9, color: GOLD, letterSpacing: 1, cursor: "pointer" }}>LOG</button>
          </div>
          {(op.labNotes || []).map(n => (
            <div key={n.id} style={{ marginTop: 6, padding: "8px 11px", borderRadius: 10, background: "rgba(8,5,22,0.6)", border: "1px solid rgba(var(--tint-rgb),0.08)" }}>
              <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.35)", marginBottom: 3 }}>{fmtT(new Date(n.ts))}</div>
  ...
  const SeasonChamber = () => {
    ...
    return (
      <div style={{ padding: "0 14px" }}>
        {/* Sun season */}
        <div style={{ borderRadius: 14, background: "rgba(8,5,22,0.85)", border: "1px solid rgba(var(--tint-rgb),0.25)", padding: "14px 15px", marginBottom: 9 }}>
          <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.45)", letterSpacing: 2.5, textTransform: "uppercase", marginBottom: 4 }}>The Season of the Work — Sun in {season.sign} {season.sym}</div>
          ...
          <div style={{ fontFamily: F, fontSize: 7.5, color: "rgba(var(--tint-rgb),0.3)", marginTop: 6 }}>The alchemical zodiac — Pernety, Dictionnaire mytho-hermétique (1758)</div>
        </div>
        ...
        <div style={{ borderRadius: 13, background: "rgba(8,5,22,0.65)", border: `1px solid ${P[dayPk].col}30`, padding: "12px 14px", marginBottom: 9 }}>
          <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.45)", letterSpacing: 2.5, textTransform: "uppercase", marginBottom: 4 }}>The Day's Counsel — {P[dayPk].sym} {P[dayPk].name}'s Day</div>
          <div style={{ fontFamily: F, fontSize: 10.5, color: "#C4A870", fontStyle: "italic", lineHeight: 1.8 }}>{DAY_OPERATION_COUNSEL[dayPk]}</div>
          <div style={{ fontFamily: F, fontSize: 9, color: "rgba(var(--tint-rgb),0.4)", marginTop: 5 }}>Metal of the day: {metal?.metal} {metal?.altGlyph} · Herbs: {P[dayPk].herbs.split("·").slice(0, 2).join("·")}</div>
        </div>
        ...
        {(() => {
          ...
          return (
            <div style={{ borderRadius: 13, background: "rgba(8,5,22,0.65)", border: "1px solid rgba(var(--tint-rgb),0.12)", padding: "12px 14px", marginBottom: 9 }}>
              <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.45)", letterSpacing: 2.5, textTransform: "uppercase", marginBottom: 6 }}>Windows Opening</div>
              {due.map(({ o, s }) => (
                <div key={s.id} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid rgba(var(--tint-rgb),0.05)" }}>
                  <span style={{ fontFamily: F, fontSize: 10, color: "#C4A870" }}><span style={{ color: P[o.planet].col }}>{P[o.planet].sym}</span> {o.name}: {s.title}</span>
                  <span style={{ fontFamily: F, fontSize: 9, color: "rgba(var(--tint-rgb),0.5)" }}>{fmtT(new Date(s.scheduledFor))}</span>
                </div>
              ))}
            </div>
          );
        ...
        {/* Timing doctrine */}
        <div style={{ borderRadius: 13, background: "rgba(8,5,22,0.65)", border: "1px solid rgba(var(--tint-rgb),0.09)", padding: "12px 14px", marginBottom: 9 }}>
          <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.45)", letterSpacing: 2.5, textTransform: "uppercase", marginBottom: 6 }}>The Timing Doctrine</div>
          {TIMING_DOCTRINE.map((t, i) => (
            <div key={i} style={{ marginBottom: 7 }}>
              <div style={{ fontFamily: F, fontSize: 10, color: GOLD }}>{t.rule}</div>
              <div style={{ fontFamily: F, fontSize: 9, color: "#8A7050", fontStyle: "italic", lineHeight: 1.6, marginTop: 1 }}>{t.text} <span style={{ color: "rgba(var(--tint-rgb),0.3)" }}>({t.source})</span></div>
            </div>
          ))}
        </div>
      </div>
  ...
  const LibraryChamber = () => {
    const SECTIONS = [
      { id: "stages", title: "The Stages of the Great Work", render: () => (<>
        {GREAT_WORK_STAGES.map(s => (
          <div key={s.key} style={{ marginBottom: 8, padding: "10px 12px", borderRadius: 11, background: "rgba(0,0,0,0.3)", borderLeft: `3px solid ${s.accent}` }}>
            <div style={{ fontFamily: F, fontSize: 12, color: s.accent }}>{s.name} <span style={{ fontSize: 8.5, color: "rgba(var(--tint-rgb),0.4)" }}>· {s.greek} · {s.epithet}</span></div>
            <div style={{ fontFamily: F, fontSize: 10, color: "#9A8060", fontStyle: "italic", lineHeight: 1.7, marginTop: 4 }}>{s.meaning}</div>
            <div style={{ fontFamily: F, fontSize: 9, color: "rgba(var(--tint-rgb),0.4)", marginTop: 3 }}>In the vessel: {s.operations}</div>
            <div style={{ fontFamily: F, fontSize: 9, color: "rgba(160,140,220,0.6)", fontStyle: "italic", marginTop: 3, lineHeight: 1.6 }}>{s.inner}</div>
          </div>
        ))}
        <div style={{ padding: "10px 12px", borderRadius: 11, background: "rgba(0,0,0,0.25)", border: "1px dashed rgba(var(--tint-rgb),0.15)" }}>
        ...
        <div style={{ fontFamily: F, fontSize: 8.5, color: "rgba(var(--tint-rgb),0.35)", fontStyle: "italic", marginTop: 8, lineHeight: 1.6 }}>{STAGE_SCHEMES_NOTE}</div>
      </>) },
      { id: "zodiac", title: "The Alchemical Zodiac (Pernety, 1758)", render: () => (<>
        {ALCHEMICAL_ZODIAC.map(z => (
          <div key={z.n} style={{ display: "flex", gap: 9, padding: "6px 0", borderBottom: "1px solid rgba(var(--tint-rgb),0.05)" }}>
            <span style={{ fontFamily: F, fontSize: 13, color: GOLD, width: 22 }}>{z.sym}</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: F, fontSize: 10.5, color: "#C4A870" }}>{z.process} <span style={{ color: "rgba(var(--tint-rgb),0.4)", fontSize: 8.5 }}>· {z.sign}</span></div>
      ...
      { id: "principles", title: "Tria Prima, Elements & Quintessence", render: () => (<>
        {TRIA_PRIMA.map(p => (
          <div key={p.key} style={{ marginBottom: 7, padding: "9px 11px", borderRadius: 10, background: "rgba(0,0,0,0.28)" }}>
            ...
            <div style={{ fontFamily: F, fontSize: 9, color: "rgba(var(--tint-rgb),0.45)", marginTop: 2 }}>In the plant work: {p.inPlants}</div>
          </div>
        ))}
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, margin: "8px 0" }}>
          {ELEMENTS.map(e => <span key={e.name} style={{ fontFamily: F, fontSize: 9.5, color: "#C4A870", padding: "4px 9px", borderRadius: 8, background: "rgba(0,0,0,0.3)", border: "1px solid rgba(var(--tint-rgb),0.12)" }}>{e.sym} {e.name} · {e.qualities}</span>)}
        </div>
        <div style={{ fontFamily: F, fontSize: 8.5, color: "rgba(var(--tint-rgb),0.35)", fontStyle: "italic", lineHeight: 1.6 }}>{TRIA_PRIMA_NOTE}</div>
      </>) },
      { id: "metals", title: "The Seven Metals & Their Planets", render: () => (<>
        {METALS.map(m => (
          <div key={m.metal} style={{ display: "flex", gap: 10, padding: "7px 0", borderBottom: "1px solid rgba(var(--tint-rgb),0.05)" }}>
            <span style={{ fontSize: 15, color: P[m.planet].col, width: 22 }}>{m.sym}</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: F, fontSize: 10.5, color: "#C4A870" }}>{m.metal} {m.altGlyph} — {P[m.planet].name} <span style={{ fontSize: 8, color: "rgba(var(--tint-rgb),0.35)" }}>(Agrippa {m.agrippa})</span></div>
      ...
      { id: "fire", title: "The Four Degrees of Fire", render: () => (<>
        {FIRE_DEGREES.map(f => (
          <div key={f.n} style={{ marginBottom: 8, padding: "9px 11px", borderRadius: 10, background: "rgba(0,0,0,0.28)" }}>
            <div style={{ fontFamily: F, fontSize: 11, color: GOLD }}>{f.glyph} {f.name} <span style={{ fontSize: 8.5, color: "rgba(var(--tint-rgb),0.4)" }}>· {f.temp}</span></div>
      ...
      { id: "tablet", title: "The Emerald Tablet (Newton's translation)", render: () => (<>
        ...
        <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.35)", textAlign: "center", marginTop: 8 }}>— {EMERALD_TABLET.translator} · {EMERALD_TABLET.source}</div>
      </>) },
      ...
      { id: "athanor", title: "The Athanor Itself", render: () => (<>
        ...
        <div style={{ fontFamily: F, fontSize: 9, color: "rgba(var(--tint-rgb),0.4)", marginTop: 6 }}>Etymology: {ATHANOR_LORE.etymology}<br />Also called: {ATHANOR_LORE.nicknames}</div>
      </>) },
      { id: "study", title: "⚠ The Study-Only Paths", render: () => (<>
        ...
        {STUDY_ONLY_PATHS.map(s => (
          <div key={s.key} style={{ marginBottom: 9, padding: "10px 12px", borderRadius: 11, background: "rgba(60,20,15,0.25)", border: "1px solid rgba(200,100,80,0.25)" }}>
            ...
            <div style={{ fontFamily: F, fontSize: 8.5, color: "rgba(var(--tint-rgb),0.4)", marginTop: 2 }}>{s.source}</div>
      ...
      { id: "glyphs", title: "The Glyphs", render: () => (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {GLYPHS.map(g => <span key={g.name} style={{ fontFamily: F, fontSize: 9.5, color: "#C4A870", padding: "5px 9px", borderRadius: 8, background: "rgba(0,0,0,0.3)", border: "1px solid rgba(var(--tint-rgb),0.1)" }}><span style={{ fontSize: 13 }}>{g.g}</span> {g.name}</span>)}
        </div>
      ) },
    ];
    return (
      <div style={{ padding: "0 14px" }}>
        {SECTIONS.map(sec => (
          <div key={sec.id} style={{ marginBottom: 7, borderRadius: 12, background: "rgba(8,5,22,0.65)", border: "1px solid rgba(var(--tint-rgb),0.1)" }}>
            <button onClick={() => setLibOpen(libOpen === sec.id ? null : sec.id)} style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "11px 14px", background: "none", border: "none", cursor: "pointer" }}>
              <span style={{ fontFamily: F, fontSize: 11, color: libOpen === sec.id ? GOLD : "rgba(var(--tint-rgb),0.6)", letterSpacing: 1 }}>{sec.title}</span>
              <span style={{ color: "rgba(var(--tint-rgb),0.35)", fontSize: 11 }}>{libOpen === sec.id ? "▾" : "▸"}</span>
            </button>
    ...
    return (
      <button onClick={() => { setSel(op); setMode("view"); }} style={{ width: "100%", textAlign: "left", marginBottom: 8, padding: "12px 13px", borderRadius: 13, background: "rgba(8,5,22,0.65)", border: `1px solid ${pl.col}25`, cursor: "pointer" }}>
        ...
        <div style={{ display: "flex", gap: 3, margin: "8px 0 6px" }}>
          {op.steps.map(s => <div key={s.id} style={{ flex: 1, height: 3, borderRadius: 2, background: s.completedAt ? pl.col : "rgba(var(--tint-rgb),0.12)" }} />)}
        </div>
        {op.status === "active" && nextStep && (
          <div style={{ fontFamily: F, fontSize: 9.5, color: "rgba(var(--tint-rgb),0.55)", fontStyle: "italic" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {chamber === "fire" && (
          <div style={{ padding: "0 14px" }}>
            ...
            {finished.length > 0 && <div style={{ fontFamily: F, fontSize: 8, color: "rgba(var(--tint-rgb),0.35)", letterSpacing: 2.5, textTransform: "uppercase", margin: "12px 0 7px" }}>Completed & Abandoned</div>}
