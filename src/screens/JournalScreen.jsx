    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",paddingBottom:20}}>
      <div style={{padding:"16px 18px 10px",display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
        ...
        <div style={{display:"flex",gap:6}}>
          ...
          <button onClick={()=>setShowNew(!showNew)} style={{padding:"8px 14px",borderRadius:10,background:"rgba(var(--tint-rgb),0.1)",border:"1px solid rgba(var(--tint-rgb),0.28)",fontFamily:F,fontSize:9,color:GOLD,letterSpacing:2,cursor:"pointer"}}>{showNew?"CANCEL":"+ LOG"}</button>
        </div>
      </div>
      {reflection&&(
        <div style={{margin:"0 14px 10px",padding:"13px 14px",borderRadius:13,background:"rgba(20,15,40,0.8)",border:"1px solid rgba(100,80,160,0.25)"}}>
          ...
          <button onClick={()=>setReflection(null)} style={{marginTop:10,background:"none",border:"none",fontFamily:F,fontSize:9,color:"rgba(var(--tint-rgb),0.3)",cursor:"pointer",letterSpacing:1}}>DISMISS</button>
        </div>
      )}
      {showNew&&<div style={{margin:"0 14px 10px",padding:"13px 14px",borderRadius:13,background:"rgba(8,5,22,0.65)",border:"1px solid rgba(var(--tint-rgb),0.1)"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
          <div><div style={{fontFamily:F,fontSize:8,color:"rgba(var(--tint-rgb),0.4)",letterSpacing:2,textTransform:"uppercase",marginBottom:4}}>Date</div><input type="date" value={form.date} onChange={e=>setForm({...form,date:e.target.value})} style={{background:"rgba(0,0,0,0.45)",border:"1px solid rgba(var(--tint-rgb),0.18)",borderRadius:10,color:"#C4A870",fontFamily:F,outline:"none",padding:"8px 10px",width:"100%",fontSize:11}}/></div>
          <div><div style={{fontFamily:F,fontSize:8,color:"rgba(var(--tint-rgb),0.4)",letterSpacing:2,textTransform:"uppercase",marginBottom:4}}>Planet</div>
            <select value={form.planet} onChange={e=>setForm({...form,planet:e.target.value})} style={{background:"rgba(0,0,0,0.45)",border:"1px solid rgba(var(--tint-rgb),0.18)",borderRadius:10,color:"#C4A870",fontFamily:F,outline:"none",padding:"8px 10px",width:"100%",fontSize:11}}>
        ...
        {[["Intention","intent","What was the working for?"],["Outcome","outcome","What happened?"]].map(([lbl,key,ph])=><div key={key} style={{marginBottom:7}}>
          <div style={{fontFamily:F,fontSize:8,color:"rgba(var(--tint-rgb),0.4)",letterSpacing:2,textTransform:"uppercase",marginBottom:4}}>{lbl}</div>
          <textarea value={form[key]} onChange={e=>setForm({...form,[key]:e.target.value})} rows={2} placeholder={ph} style={{background:"rgba(0,0,0,0.45)",border:"1px solid rgba(var(--tint-rgb),0.18)",borderRadius:10,color:"#C4A870",fontFamily:F,outline:"none",padding:"8px 10px",width:"100%",fontSize:11,resize:"none"}}/>
        </div>)}
        <button onClick={save} disabled={!form.intent} style={{width:"100%",padding:"10px 0",borderRadius:10,background:form.intent?"rgba(var(--tint-rgb),0.1)":"rgba(0,0,0,0.3)",border:"1px solid "+(form.intent?"rgba(var(--tint-rgb),0.3)":"rgba(var(--tint-rgb),0.1)"),fontFamily:F,fontSize:9,color:form.intent?GOLD:"#5A4020",letterSpacing:2,textTransform:"uppercase",cursor:form.intent?"pointer":"default"}}>Save Entry</button>
      </div>}
      <div style={{flex:1,overflowY:"auto",padding:"0 14px"}}>
        ...
        entries.map(e=>{const pl=P[e.planet];return(<div key={e.id} style={{marginBottom:9,padding:"12px 13px",borderRadius:13,background:"rgba(8,5,22,0.65)",border:"1px solid "+pl.col+"17"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            ...
            </div><div style={{fontFamily:F,fontSize:12,color:GOLD,marginTop:3,fontStyle:"italic"}}>{e.intent}</div></div>
            <button onClick={()=>del(e.id)} style={{background:"none",border:"none",color:"rgba(var(--tint-rgb),0.25)",cursor:"pointer",fontSize:12}}>✕</button>
          </div>
