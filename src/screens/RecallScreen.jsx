  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", paddingBottom: 20 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 14px" }}>
        <input value={q} onChange={e => setQ(e.target.value)} autoFocus placeholder="Ask your record — a planet, an intent, a phase…" style={{ width: "100%", background: "rgba(0,0,0,0.5)", border: "1px solid rgba(200,175,100,0.25)", borderRadius: 11, color: "#D4C098", fontFamily: F, outline: "none", padding: "12px 14px", fontSize: 13, boxSizing: "border-box", marginBottom: 9 }} />
        ...
        {rag.count > 0 && !q.trim() && (
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 4 }}>
            {SUGGESTIONS.map(s => <button key={s} onClick={() => setQ(s)} style={{ fontFamily: F, fontSize: 9, color: "rgba(200,175,100,0.6)", background: "rgba(0,0,0,0.3)", border: "1px solid rgba(200,175,100,0.15)", borderRadius: 14, padding: "5px 11px", cursor: "pointer" }}>{s}</button>)}
          </div>
        )}
        ...
        {results.map((r, i) => {
          ...
          return (
            <div key={i} style={{ marginBottom: 8, padding: "11px 13px", borderRadius: 12, background: "rgba(8,5,22,0.6)", border: "1px solid rgba(200,175,100,0.1)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                ...
                {r.doc.date && <span style={{ fontFamily: F, fontSize: 8.5, color: "rgba(200,175,100,0.4)" }}>{r.doc.date}</span>}
                {setTab && r.doc.tab && <button onClick={() => setTab(r.doc.tab)} style={{ marginLeft: "auto", fontFamily: F, fontSize: 8, color: "rgba(200,175,100,0.55)", background: "none", border: "1px solid rgba(200,175,100,0.18)", borderRadius: 6, padding: "2px 8px", letterSpacing: 1, textTransform: "uppercase", cursor: "pointer" }}>Open →</button>}
              </div>
